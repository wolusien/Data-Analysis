...
Lors de la normalisation des variables, les données peuvent être transformées comme suit: $\frac{x_{i}-mean(x)}{sd(x)}$ Où $mean(x)$ est la moyenne des valeurs de x, et $sd(x)$ est l’écart type (SD).

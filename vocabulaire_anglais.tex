...
\begin{itemize}[label=\textbullet]
	...
	\item \textbf{data deduplication} = \textbf{\textit{Déduplication de données}} : La déduplication des données, aussi appelée « compression intelligente » ou « stockage d'instance unique », réduit les besoins de stockage en supprimant les données redondantes.\\Le support de stockage, disque ou bande, conserve donc une seule instance des données. Les données redondantes sont remplacées par un pointeur vers l'unique copie.
	\item \textbf{Fan out} = \textbf{\textit{Fan out}} is the term for delivering events from one source to multiple channels, so they reach multiple sinks.
	\item \textbf{RDBMS(Relational Database Management System)} = \textbf{Système de gestion de base de données relationelle}
	\item \textbf{JDBC(Java Database Connectivity}) = Interface de programmation pour les programmes utilisant la plateforme Java. Elle permet aux applications Java d'accéder par le biais d'une interface commune à des sources de données pour lesquelles il existe des pilotes JDBC. Normalement, il s'agit d'une base de données relationnelle, et des pilotes JDBC sont disponibles pour tous les systèmes connus de bases de données relationnelles. 
	\end{itemize}
\end{document}
